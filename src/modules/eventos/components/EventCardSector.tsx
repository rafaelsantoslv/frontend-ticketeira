...
import type { Sector } from "@/modules/eventos/types"

type Props = {
    sector: Sector
    isExpanded: boolean
    toggleSectorExpanded: (id: string) => void
    setIsBatchFormOpen: (value: boolean) => void
    handleDeleteSector: (id: string) => void
    handleDeleteBatch: (id: string) => void
    handleToggleBatchStatus: (id: string) => void
}
...
export function EventCardSector({
    ...
    const totalSold = sector.batches.reduce((acc, b) => acc + b.sold, 0)
    const totalRevenue = sector.batches.reduce(
        (acc, b) => acc + b.sold * b.price,
        0
    )
    ...
    return (
        <Card key={sector.id} className="overflow-hidden">
            <Collapsible
                open={isExpanded}
                onOpenChange={() => toggleSectorExpanded(sector.id)}
            >
                <div className="p-4 flex items-center justify-between bg-gray-50 border-b">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                Capacidade: {sector.capacity ?? "?"} • Vendidos: {totalSold} • Receita:{" "}
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button
                            ...
                            disabled={sector.batches.length > 0}
                        >
                                    ...
                                    >
                                        <div>
                                            <div className="flex items-center">
                                                ...
                                                <span
                                                    className={`ml-2 text-xs px-2 py-0.5 rounded-full ${batch.active
                                            ...
                                            <div className="text-sm text-gray-500">
                                                ...
                                                {batch.quantity} ingressos vendidos
                                            </div>
                                        </div>
